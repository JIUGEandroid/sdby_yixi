...
class selectKuguan extends Component {
	constructor(props) {
	  super(props);
	...
	getData(msg){
		let postData={"role":"3"};
		if(msg){
			postData={"role":"3","username":msg};
		}
		Tools.post("http://yixip.bowyue.com/api/pmember/index",postData,(resData)=>{
			this.closeLoading();
			console.log("selectKuguan:"+JSON.stringify(resData));
	        if(resData.length>0){
	...
	renderRow(rowData,i,j){
		return(
			<TouchableOpacity onPress={()=>this.backKuguanInfo(rowData)} activeOpacity={0.5}>
    ...
    return (
      <View style={{flex:1,backgroundColor:'#fff'}}> 
      	<View style={styles.container}>
      		<NavTitle title={"选择库管"} goBack={this.goBack.bind(this)}/>
